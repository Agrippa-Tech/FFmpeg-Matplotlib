...
import threading
import time
from typing import Optional, Dict, Any, Callable, Set, List, Tuple, Final
...
COMMON_CODECS: Final[Set[str]] = {
    'libx264', 'libx265', 'mpeg4', 'h264', 'vp9',
    'h264_nvenc', 'hevc_nvenc', 'libvpx', 'libvpx-vp9'
}
...
class FFmpegValidator:
    ...
    def validate_executable(cls, path: str) -> ValidationResult:
        ...
        if not resolved_path:
            return ValidationResult(
                is_valid=False,
                error_message=f"Executável não encontrado: {path}"
            )
        ...
        try:
            ...
            if 'ffmpeg version' not in result.stdout.lower():
                return ValidationResult(
                    is_valid=False,
                    error_message="Executável não parece ser FFmpeg"
                )
            ...
            return ValidationResult(
                is_valid=False,
                error_message=f"Erro ao validar FFmpeg: {e}"
            )
    ...
    def query_available_codecs(cls, ffmpeg_path: str) -> CodecQueryResult:
        ...
        try:
            ...
            logger.warning(
                "Erro ao consultar codecs: %s. Usando fallback.", e
            )
            return CodecQueryResult(
                codecs=COMMON_CODECS.copy(),
                using_fallback=True,
...
class FFmpegConfig:
    ...
    def using_fallback_codecs(self) -> bool:
        ...
        """
        return (
            self._codec_cache is not None and 
            self._codec_cache.using_fallback
        )
    ...
    def _get_available_codecs(self) -> CodecQueryResult:
        ...
        with self._lock:
            ...
            # Consultar codecs
            self._codec_cache = self.validator.query_available_codecs(
                self._ffmpeg_path
            )
    ...
    def _create_verbose_callback(
        self,
        user_callback: Optional[Callable[[int, int], None]] = None
    ...
    def save_animation(
        ...
        **kwargs
        ...
        if options.check_disk_space:
            ...
            try:
                total_frames = len(animation._func_handles) if hasattr(animation, '_func_handles') else 100
                duration = total_frames / options.fps
                
                fig = animation._fig if hasattr(animation, '_fig') else None
                resolution = None
                if fig:
                    resolution = (int(fig.get_figwidth() * dpi), int(fig.get_figheight() * dpi))
                ...
                if disk_info.available_mb - disk_info.required_mb < disk_info.warning_threshold_mb:
                    logger.warning(
